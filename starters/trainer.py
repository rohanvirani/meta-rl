...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            # get final goal
            final_state, final_action, final_reward, final_next_state, final_goal = episode_experience[-1]

            # compute new reward
            
            new_reward = env_reward_function(final_next_state, state)

            # add to buffer
            replay_buffer.add(np.append(state, final_next_state),
                          action,
                          new_reward,
                          np.append(next_state, final_next_state))
            ...
            random_future_time = np.random.randint(timestep,len(episode_experience))

            random_state, random_action, random_reward, random_next_state, random_goal = episode_experience[random_future_time]

            # compute new reward
            new_reward = env_reward_function(random_next_state, state)

            # add to replay buffer
            replay_buffer.add(np.append(state, random_next_state),
                          action,
                          new_reward,
                          np.append(next_state, random_next_state))
            ...
            random_future_time = np.random.randint(0,len(episode_experience))
