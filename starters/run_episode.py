...
def run_episode(
    ...
    for _ in range(steps_per_episode):
        ...
        #print(state)
        #print(goal_state)
        input = np.concatenate((state, goal_state), axis=0)

        # forward pass to find action

        action = torch.argmax(q_net.forward(torch.Tensor(input)))
        #print(action)
        ...
        episode_experience.append((state, action, reward, next_state, info['goal_vector']))
